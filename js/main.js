...
window.addEventListener("DOMContentLoaded", function(){
	...
	function storeData(key){
		...
		item.pw				= ["Desired Password:", $('pw').value];
		item.email 			= ["Email:" , $('email').value];
		item.pn				= ["Phone Number:", $('pn').value];
	...
	function getData(){
		...
		if(localStorage.length === 0){
			alert("There are no saved entries. Would you like to use sample data?");
	...
	function editItem(){
		...
		$('range').value = item.ease[1];//Ease of Use information
	...
	function validate(e){
		...
		if(getUsername.value ===""){
			var usernameError = "Please enter a username for this site.";
			getUsername.style.border = "2px solid red";
